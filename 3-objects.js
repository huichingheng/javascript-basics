...
function listTitles(booksArray) {
  return booksArray.map(books => books.title);
}
// y i put like this is also the same:
...
function listAuthors(booksArray) {
  return booksArray.map(books => books.author);
}
...
function listValues(x, key) {
  return x.map(y => y[key]);
}
